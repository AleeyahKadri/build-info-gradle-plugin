...
fun includeProject(projectDirName: String, projectName: String) {
    ...
    require(projectDir.isDirectory) { "Project directory $projectDir does not exist" }
    require(File(projectDir, buildFileName).isFile) { "Build file $buildFileName does not exist in $projectDir" }
...
listOf("docs", "subprojects").forEach { dirName ->
    ...
    subdir.listFiles()?.filter { it.isDirectory }?.forEach { dir ->
